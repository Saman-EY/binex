    ...
    <>
      {items.map((item, i) => {
        ...
        const isOrHasActiveLink = ((item.link && item.link === activeLink) ||
          (item.children && item.children.find((q) => q.link && q.link === activeLink))) as boolean;
              ...
              <>
                ...
                >
                  <div
                    ...
                    onClick={() => {
                      i === 0 && router.push(routes.dashboard.myProfitLoss.foreign);
                    }}
                  >
                    {item.icon}
                    <span className="ml-auto text-sm font-semibold">{item.text}</span>
                    {!!item.children && i > 0 && (
